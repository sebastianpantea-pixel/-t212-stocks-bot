...
# cache TTL separat fata de rate limit
T212_CACHE_TTL = {
...
def clean_closed_positions():
    # pastreaza doar ultimele 200 in memorie
    ...
    with t212_lock:
        for attempt in range(retries + 1):
            try:
                ...
                # dupa ordin, invalideaza cache-ul relevant
...
def get_price(symbol):
    try:
        ...
        r.raise_for_status()
        q = r.json().get("quote", {})
        ask = safe_float(q.get("ap"), 0.0)
        bid = safe_float(q.get("bp"), 0.0)
        if ask > 0 and bid > 0:
...
def close_trade(pos, reason):
    ...
    add_log(
        f"{'✅' if pnl > 0 else '❌'} {pos['symbol']} {reason} | ${pnl:+.2f} | {rr}",
...
def sync_positions():
    ...
    for pos in state["positions"]:
        ...
        if live:
            ...
            # pozitie inchisa extern sau inca nesincronizata
            # aici preferam sa nu o inchidem imediat fals pozitiv
            pass
...
def bot_loop():
    ...
    while state["running"]:
        try:
            ...
            # sync portfolio relativ des, dar prin cache/rate limit
            sync_positions()
